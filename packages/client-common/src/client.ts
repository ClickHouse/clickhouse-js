...
export class ClickHouseClient<Stream = unknown> {
  ...
  async query<Format extends DataFormat = "JSON">(
    ...
    try {
      const { stream, query_id, response_headers } = await this.withActiveSpan(
        span,
        () =>
      ...
      span.setAttributes({ "clickhouse.query_id": query_id });
      span.setStatus({ code: ClickHouseSpanStatusCode.OK });
  ...
  async command(params: CommandParams): Promise<CommandResult> {
    ...
    try {
      const result = await this.withActiveSpan(span, () =>
      ...
      span.setAttributes({ "clickhouse.query_id": result.query_id });
      span.setStatus({ code: ClickHouseSpanStatusCode.OK });
      return result;
    } catch (err) {
      recordSpanError(span, err);
  ...
  async ping(params?: PingParams): Promise<PingResult> {
    ...
    try {
      const result = await this.withActiveSpan(span, () =>
        this.connection.ping(params ?? { select: false }),
      );
      span.setStatus({ code: ClickHouseSpanStatusCode.OK });
      return result;
    } catch (err) {
  ...
  /** Starts a tracer span for a client operation, or returns the shared
   *  no-op span when no tracer is configured. */
  private startSpan(
    name: string,
  ...
  /** Runs `fn` inside the tracer's active-span scope (if it provides one),
   *  so auto-instrumented child spans are parented under `span`. */
  private withActiveSpan<T>(span: ClickHouseSpan, fn: () => T): T {
    return this.tracer?.withActiveSpan !== undefined
      ? this.tracer.withActiveSpan(span, fn)
      : fn();
  }
...
function recordSpanError(span: ClickHouseSpan, err: unknown): void {
