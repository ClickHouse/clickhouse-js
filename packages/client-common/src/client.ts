...
import { ClickHouseSpanNames } from './tracing'
import { runWithTracer } from './tracing_internal'
...
export class ClickHouseClient<Stream = unknown> {
  ...
  private readonly tracer?: ClickHouseTracer<unknown>
  ...
  async command(params: CommandParams): Promise<CommandResult> {
    ...
    return runWithTracer(
      ...
      (result) => ({ 'clickhouse.query_id': result.query_id }),
    )
  }
