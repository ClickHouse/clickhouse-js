...
describe("data types", () => {
  ...
  it("cannot insert raw binary buffer data into FixedString via JSONEachRow (#837)", async () => {
    const table = await createTableWithFields(client, "fs FixedString(2)");
    // Two bytes that fit into FixedString(2) as raw bytes, but become four
    // bytes once encoded as UTF-8 text (each 0xF0 turns into 0xEF 0xBF 0xBD).
  ...
  it("silently corrupts binary buffer data inserted into String via JSONEachRow (#837)", async () => {
