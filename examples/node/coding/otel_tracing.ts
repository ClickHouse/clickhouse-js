...
//    `ClickHouseSpanNames`) carrying the stable OTEL database semantic
//    convention attributes such as `db.system.name`, `db.operation.name`,
//    `server.address`/`server.port`, and the server-assigned
//    `clickhouse.query_id`.
...
for (const span of spans) {
  console.info("[OtelTracing] Span:", {
    ...
    "db.operation.name": span.attributes["db.operation.name"],
...
// user land instead of being baked into the client.
// ---------------------------------------------------------------------------
