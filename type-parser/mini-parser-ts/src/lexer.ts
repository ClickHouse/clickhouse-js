...
function decodeQuoted(input: string, pos: number, quote: string): DecodeResult {
  ...
  while (pos < n) {
    ...
    if (c === "\\") {
      if (pos + 1 >= n) {
        return { ok: false, out, error: "unterminated escape in quoted literal", pos }
      }
...
export function tokenize(input: string): Token[] {
  ...
  const fail = (at: number, msg: string): void => {
    tokens.push({ type: TokenType.Error, text: msg, is_float: false, begin: at })
  }
  ...
  while (pos < n) {
    ...
    switch (c) {
      case "(":
        tokens.push({ type: TokenType.OpeningParen, text: "(", is_float: false, begin: start })
        ++pos
        continue
      case ")":
        tokens.push({ type: TokenType.ClosingParen, text: ")", is_float: false, begin: start })
        ++pos
        continue
      case ",":
        tokens.push({ type: TokenType.Comma, text: ",", is_float: false, begin: start })
        ++pos
        continue
      case "=":
        tokens.push({ type: TokenType.Equals, text: "=", is_float: false, begin: start })
        ++pos
        continue
      case "-":
        tokens.push({ type: TokenType.Minus, text: "-", is_float: false, begin: start })
        ++pos
        continue
      default:
        break
    ...
    if (c === "." && !(pos + 1 < n && isDigit(input[pos + 1] as string))) {
      tokens.push({ type: TokenType.Dot, text: ".", is_float: false, begin: start })
      ++pos
      continue
    }
    ...
    if (c === "`" || c === '"') {
      ...
      tokens.push({ type: TokenType.QuotedIdent, text: r.out, is_float: false, begin: start })
      continue
    }
    ...
    if (c === "'") {
      ...
      tokens.push({ type: TokenType.String, text: r.out, is_float: false, begin: start })
      continue
    }
    ...
    if (isDigit(c) || c === ".") {
      ...
      tokens.push({ type: TokenType.Number, text: input.substring(start, pos), is_float, begin: start })
      continue
    }
    ...
    if (isWordFirst(c)) {
      ...
      tokens.push({ type: TokenType.Word, text: input.substring(start, pos), is_float: false, begin: start })
      continue
    }
