...
describe('HttpAdapter', () => {
  ...
  describe('compression', () => {
    describe('general settings and gzip', () => {
      describe('response decompression', () => {
        it('hints ClickHouse server to send a gzip compressed response if compress_response: true', async () => {
        ...
        it('does not send a compression algorithm hint if compress_request: false', async () => {
          ...
          assertDecompressStub(undefined)
        })
    ...
    describe('lz4', () => {
      it('hints ClickHouse server to send an lz4 compressed response if compress_response: true', async () => {
        const adapter = buildHttpAdapter({
          compression: {
            decompress_response: true,
            compress_request: false,
      ...
      it('sends a compressed request if compress_request: true', async () => {
        ...
        const request = new Stream.Writable({
          ...
          final() {
            finalResult = lz4.uncompressSync(chunks)
          },
        }) as ClientRequest
      ...
      async function emitCompressedBody(
        ...
        const compressedBody = lz4.compressSync(body)
        request.emit(
          'response',
          buildIncomingMessage({
            body: compressedBody,
  ...
  function assertCompressStub(encoding: string | undefined) {
  ...
  function assertDecompressStub(encoding: string | undefined) {
    expect(httpRequestStub).toBeCalledTimes(1)
    const calledWith = httpRequestStub.mock.calls[0][1]
